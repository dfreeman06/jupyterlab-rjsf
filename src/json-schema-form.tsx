...
export class JSONSchemaFormModel extends BoxModel {
...
export class JSONSchemaFormView extends BoxView {
  ...
  async initForm() {
    const { m, onChange, idPrefix, onError} = this;
    const { formData, schema, uiSchema } = m;
    ...
    if (JSONSchemaFormView._rendermime) {
      options = {
        markdown: JSONSchemaFormView._rendermime.createRenderer(
          'text/markdown'
        ) as unknown as RenderedMarkdown,
      };
    }
    ...
    this.luminoWidget.addWidget(this._form);
  }
